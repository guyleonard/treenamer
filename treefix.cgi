...
sub svg {
    ...
    open my $in, '<', "$UPLOAD_DIR\/results\/tf\/$ID\/$tree_filename"
        or die("Error in conversion SVG: Unable to open file: $tree_filename: $!\n");

    my ( $tree_file_out, $tree_dir_out, $tree_ext_out ) = fileparse( "$tree_filename", qr"\..*" );

    open my $svg_out, '>', "$UPLOAD_DIR\/results\/tf\/$ID\/$tree_file_out\_fixed$tree_ext_out"
        or die("Error: Unable to write svg file $tree_file_out\_fixed$tree_ext_out: $!\n");
    while (<$in>) {
        my $line  = $_;
        my $label = $QUERY->param("label");
        my $block = $QUERY->param("block");
        given ($label) {
			...
			# modified here to include a newline that I removed earlier...
			# hoping this fixes the "all on one line" bug....
			# same with all print out $line's below...
                        print $svg_out "$line";
                    }
                    when ('short') {
                        for ( my $i = 0; $i <= $#REF_ARRAY; $i++ ) {
...
<body>
    ...
    <fieldset>
        ...
        <fieldset>
                ...
                <ol>
        <li>SVG: <a href="/results\/tf/$ID/$file_name\_fixed\.svg">SVG_TREE</a></li>
